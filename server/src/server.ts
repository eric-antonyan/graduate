...
const MONGO_URI =
	...
	"mongodb+srv://eric:AHsPF8cf@cluster0.sldmkon.mongodb.net/graduates?retryWrites=true&w=majority";
const CLIENT_ORIGIN =
	process.env.CLIENT_ORIGIN || "https://graduate-3qb9.vercel.app";
const JWT_SECRET = process.env.JWT_SECRET || "CHANGE_THIS_SECRET_IN_ENV";
const ADMIN_SECRET = process.env.ADMIN_SECRET || "dasaran$";
const EVENT_CLOSE_AT = new Date(
	process.env.EVENT_CLOSE_AT || "2026-05-26T00:00:00+04:00",
);
	...
	asyncHandler(async (req, res) => {
		...
		if (!phone?.trim() || !adminSecret?.trim()) {
			return res
				...
				.json({
					success: false,
					message: "Phone and admin secret are required.",
				});
		}
...
app.get(
	...
	asyncHandler(async (_req, res) => {
		const [visits, members, pendingMembers, approvedMembers] =
			await Promise.all([
